...
export default function FCM() {
  ...
  const setCurrentToken = useUserStore((state) => state.setCurrentToken); // Zustand에서 setCurrentToken 가져오기
  ...
  const onMessageFCM = useCallback(async () => {
    ...
    getToken(messaging, {
      vapidKey:
        "BHzDuDNOeit-xrwflV5FULJ_AbFc17dnJVTVrmeX6FJUYD6DJnIMXTHK3dQtvwgwlCn1YxcwW-AoU7XgvMe_0YU",
    })
      .then((token) => {
        if (token) {
          currentToken.current = token; // useRef로 관리하는 currentToken에 값 저장
          setCurrentToken(token); // Zustand의 currentToken 업데이트
  ...
  }, [userInfo, setCurrentToken, checkAndRegisterToken]); // checkAndRegisterToken을 의존성 배열에 추가

  const unregisterTokenFromServer = useCallback(
    async (token: string) => {
      try {
        await apiClient(`/fcm`, {
          method: "DELETE",
  ...
  }, [userInfo, onMessageFCM]); // onMessageFCM과 unregisterTokenFromServer 추가
  return null; // UI 없음
}
